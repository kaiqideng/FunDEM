...
private:
    ...
    HostDeviceArray1D<int> ghostGridHashEnd_;
...
public:
    void initialize(const LSParticle& LS, const size_t maxGPUThread, cudaStream_t stream)
    {
        ...
        const size_t numBoundaryNode = LS.boundaryNodeInfo_.num();
        ...
        ghostGridHashEnd_.allocateDevice(numGrid, stream);
    ...
    double deviceMemoryGB() const
    {
        ...
        total += ghostGridHashEnd_.deviceMemoryGB();
    ...
    void buildInteraction(LSParticle& LS, const size_t maxGPUThread, cudaStream_t stream)
    {
        ...
        LS.boundaryNodeInfo_.localPositionID(), 
        LS.boundaryNodeInfo_.particleID(), 
        LS.geometryInfo_.boundaryNodePosition(), 
        LS.position(), 
        LS.orientation(), 
        LS.geometryInfo_.gridNodeOrigin(), 
        LS.geometryInfo_.gridNodeInverseSpacing(), 
        LS.geometryInfo_.gridNodeSize(), 
        LS.geometryInfo_.SDFPrefixSum(), 
        LS.geometryInfo_.signedDistanceField(), 
        ghostPosition_.d_ptr, 
    ...
    void addForceTorque(LSParticle& LS, const double timeStep, cudaStream_t stream)
    {
        ...
        LS.geometryID(),
        LS.materialID(),
        LS.geometryInfo_.inverseMass(),
        LS.materialInfo_.normalStiffness(),
        LS.materialInfo_.shearStiffness(),
        LS.materialInfo_.frictionCoefficient(),
        LS.materialInfo_.restitutionCoefficient(),
        ghostPosition_.d_ptr, 
